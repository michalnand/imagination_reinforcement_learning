...
class AgentDDPGImaginationRNN():
    ...
    def train_model(self):
        ...
        state_seq_t, action_seq_t, state_next_t_ = self.experience_replay.sample_sequence(self.batch_size, self.sequence_length, self.model_forward.device)

        curiosity_t = self._curiosity(state_seq_t, state_next_t_, action_seq_t).detach()
        entropy_t   = self._entropy(state_seq_t, action_seq_t, self.epsilon).detach()
        ...
        #train forward model
        state_next_predicted_t = self.model_forward(state_seq_t, action_seq_t)

        loss_forward    = ((state_next_t_.detach() - state_next_predicted_t)**2)
    ...
    def _curiosity(self, state_seq_t, state_next_t, action_seq_t):
        state_predicted_t = self.model_forward(state_seq_t, action_seq_t)
    ...
    def _entropy(self, state_seq_t, action_seq_t, epsilon):
        #fill initial state
        states_initial_t  = torch.zeros((self.rollouts, self.batch_size, self.sequence_length) + self.state_shape).to(state_seq_t.device)
        for i in range(self.rollouts):
            states_initial_t[i] = state_seq_t.clone()


        #fill initial actions
        actions_initial_t  = torch.zeros((self.rollouts, self.batch_size, self.sequence_length, self.actions_count)).to(state_seq_t.device)
        for i in range(self.rollouts):
            actions_initial_t[i] = action_seq_t.clone()

        #sample actions, replace last action in actions_initial_t sequence with new action
        #for loop in rollouts is necessary when noisy nets used
        
        state_t = state_seq_t[:, -1, :]
        for i in range(self.rollouts):
            actions_t_, _                   = self._sample_action(state_t, epsilon)
            actions_initial_t[i][:, -1]     = actions_t_.clone()

        #create one big batch for faster run
        states_initial_t    = states_initial_t.reshape((self.rollouts*self.batch_size, self.sequence_length) + self.state_shape)

        actions_initial_t   = actions_initial_t.reshape((self.rollouts*self.batch_size, self.sequence_length, self.actions_count))

        #compute predicted state
        state_predicted_t = self.model_forward(states_initial_t, actions_initial_t)
