...
class AgentDQNImaginationOnLine():
    ...
    def main(self, show_activity = False):
        ...
        if self.enabled_training:
            ...
            curiosity_t = self._curiosity(state_t, state_next_t, action_idx_np).detach()
            entropy_t   = self._entropy(state_t, self.epsilon).detach()

            curiosity_np    = curiosity_t.squeeze(0).to("cpu").numpy()
            entropy_np      = entropy_t.squeeze(0).to("cpu").numpy()
            
            #predict next state, and compute forward model loss 
            state_next_predicted_t = self.model_forward(state_t, action_one_hot)
            ...
            self.entropy        = (1.0 - k)*self.entropy        + k*entropy_np
            self.curiosity      = (1.0 - k)*self.curiosity      + k*curiosity_np

            #sum external reward with internal motivation
            reward_sum = reward + curiosity_np + entropy_np

            self.experience_replay.add(self.state, action, reward_sum, done)
    ...
    def _curiosity(self, state_t, state_next_t, action_t, threshold = 0.01):
    ...
    def _entropy(self, state_t, epsilon):
        ...
        batch_size = state_t.shape[0]
        #fill initial state
        states_initial_t  = torch.zeros((self.rollouts, batch_size) + self.state_shape).to(state_t.device)
        for i in range(self.rollouts):
            states_initial_t[i] = state_t.clone()

        #sample actions
        #for loop in rollouts is necessary when noisy nets used
        actions_t  = torch.zeros((self.rollouts, batch_size, self.actions_count)).to(state_t.device)
        for i in range(self.rollouts):
            _, actions_t_ = self._sample_action(state_t, epsilon)
            actions_t[i]  = actions_t_.clone()

        #create one big batch for faster run
        states_initial_t = states_initial_t.reshape((self.rollouts*batch_size, ) + self.state_shape)

        actions_t = actions_t.reshape((self.rollouts*batch_size, self.actions_count))
        ...
        state_next_predicted_t = state_next_predicted_t.reshape((self.rollouts, batch_size, ) + self.state_shape)
