...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_shape, outputs_count, residual_count = 32, kernels_count = 256):
        ...
        for n in range(residual_count): 
            init_range = (1.0/(1 + n))**0.5
            self.layers_features.append(ResidualBlock(kernels_count, init_range))
        ...
        self.layers_value = [
            ...
            nn.Linear(8*input_height*input_width, 256),
            nn.ReLU(),
        ...
        self.layers_advantage = [
            ...
            libs_layers.NoisyLinear(8*input_height*input_width, self.outputs_count)
        ]
...
if __name__ == "__main__":
    ...
    model = Model((channels, height, width), actions_count, residual_count=32, kernels_count=256)


    q_values = model.forward(state)

    print(q_values)
    print(q_values.abs().mean(), q_values.min(), q_values.max())
