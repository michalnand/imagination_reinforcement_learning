...
class Model(torch.nn.Module):
    def __init__(self, input_shape, outputs_count, kernels_count = 64):
        ...
        self.layers = [
            nn.Conv2d(input_shape[0] + outputs_count, kernels_count, kernel_size=8, stride=4, padding=0),
            nn.ReLU(),

            ResidualBlock(kernels_count),
            ResidualBlock(kernels_count),
            
            nn.ConvTranspose2d(kernels_count, kernels_count, kernel_size=8, stride=4, padding=0),
            nn.ReLU(),
            nn.Conv2d(kernels_count, input_shape[0], kernel_size=1, stride=1, padding=0)
        ]
