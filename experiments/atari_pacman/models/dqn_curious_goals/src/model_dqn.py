...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_shape, outputs_count):
        ...
        self.layers_advantage = [
            libs_layers.NoisyLinearFull(fc_inputs_count, 512),
            nn.ReLU(),                      
            libs_layers.NoisyLinearFull(512, outputs_count)
        ]
    ...
    def save(self, path):
        ...
        torch.save(self.model_features.state_dict(), path + "trained/model_dqn_features.pt")
        torch.save(self.model_value.state_dict(), path + "trained/model_dqn_value.pt")
        torch.save(self.model_advantage.state_dict(), path + "trained/model_dqn_advantage.pt")

    def load(self, path):
        print("loading ", path) 

        self.model_features.load_state_dict(torch.load(path + "trained/model_dqn_features.pt", map_location = self.device))
        self.model_value.load_state_dict(torch.load(path + "trained/model_dqn_value.pt", map_location = self.device))
        self.model_advantage.load_state_dict(torch.load(path + "trained/model_dqn_advantage.pt", map_location = self.device))
